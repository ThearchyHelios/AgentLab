...
@pytest.fixture
async def cobook(tmp_path):
    """一个真实的小库 + 一条 DataSource 记录，工具要能真的查到数。"""
    path = tmp_path / "cobook.db"
    ...
    name = f"cobook_{tmp_path.name[-6:].replace('-', '_')}"
...
async def test_serial_loop_runs_one_tool_and_answers_every_call(runner, cobook, monkeypatch) -> None:
...
async def test_bad_args_feed_the_schema_back_instead_of_dying(runner, cobook, monkeypatch) -> None:
