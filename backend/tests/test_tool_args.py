...
def source(tmp_path):
    ...
    return SimpleNamespace(
        ...
        id="tool-args-src", name="shop", kind="sqlite", database=str(path),
...
def _calls() -> list[dict]:
    return [
        {"name": "db_schema__shop", "args": {}, "id": "c1"},
        {"name": "db_query__shop", "args": {"sql": "SELECT 1"}, "id": "c2"},
        {"name": "db_query__shop", "args": {"sql": "SELECT 2"}, "id": "c3"},
    ]
...
def test_the_skip_note_never_becomes_the_answer() -> None:
    ...
    messages = [
        HumanMessage(content="Shop 有几个管理员？"),
